...
demo_root="$(mktemp -d "${TMPDIR:-/tmp}/claude-router-demo.XXXXXXXX")"
...
export CLR_DEMO_INSTALL_URL="${CLR_DEMO_INSTALL_URL:-https://andresperez.github.io/claude-router/install.sh}"
...
export CLR_DEMO_KEY_FILE="$demo_key_file"
export CLR_DEMO_ROOT="$demo_root"
export CLR_DEMO_SNAPSHOT_FILE="$demo_root/demo-final.ansi"
...
if [[ ! -s "$claude_credentials_source" ]]; then
  ...
  echo "run claude /login or set CLR_DEMO_CLAUDE_CREDENTIALS_FILE" >&2
  exit 2
fi
...
asciinema rec \
  ...
  --overwrite \
    ...
    --rows 24 \
  --idle-time-limit 3 \
  --title "Claude Router — curl install to live GLM-5.3 response" \
...
if [[ -s "$CLR_DEMO_SNAPSHOT_FILE" ]]; then
  stabilize_args+=(--snapshot "$CLR_DEMO_SNAPSHOT_FILE")
fi
...
selected_model="$(python3 -c 'import json, sys; print(",".join(json.load(open(sys.argv[1], encoding="utf-8"))["favorites"]))' "$XDG_CONFIG_HOME/claude-router/config.json")"
